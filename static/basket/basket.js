$(document).ready(function(){
    ...
    $('.add_to_basket').click(function(){
		...
		$.post('/basket/basket.php', { id: productId, action: 'addtobasket' }, function(result) {
            quantity_goods();
	...
	$('.add_basket').click(function(){
		...
		//console.log(count)
		$.post('/basket/basket.php', { id: productId, count: count, action: 'addtobasket' }, function(result) {
            quantity_goods();
	...
	$(document).on('click', '.product-remove', function(){
		...
		$.post('/basket/basket.php', { id: iid, action: 'remove' }, function(rem) {
          if(rem == 1){
            $('.block-'+ iid).fadeOut();
            quantity_goods();
            loadallprice();
	...
	$(document).on('click', '.count-minus', function(){
		var iid = $(this).attr("iid");
		$.post('/basket/basket.php', { bid: iid, action: 'minus' }, function(min) {
	...
	$(document).on('click', '.count-plus', function(){
		var iid = $(this).attr("iid");
		$.post('/basket/basket.php', { bid: iid, action: 'plus' }, function(max) {
	...
	$(document).on('keypress', '.count-input', function(e){
		...
		if (e.keyCode == 13){
			...
			//console.log(incount);
            $.post('/basket/basket.php', { bid: iid , count: incount, action: 'countinput' }, function(cinp) {
    ...
    $(document).on('click', '.clear-cart', function(){
        $.post('/basket/basket.php', { clear: 'clear' }, function(data){
            if(data == 1){
                quantity_goods();
    ...
    **/
	function quantity_goods(){
      	$.post('/basket/basket.php', {action: 'quantity'}, function(quantity) {
...
*/
  ...
  **/
	function loadallprice(){
    	$.post('/basket/basket.php', {action: 'loadallprice'}, function(data) {
    ...
    **/
    function producttobacket(){
      	$.post('/basket/basket.php', {action: 'producttobacket'}, function(data) {
  ...
  **/
	function loadbasket(){
        $.post('/basket/basket.php', {action: 'loadbasket'}, function(data) {
        	//console.log(data);
            if(data == 0){	
                $('.basket-empty').show();	
                $('.recalculate-form').hide();
